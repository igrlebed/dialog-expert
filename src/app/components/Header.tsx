      ...
      >
        <div className="max-w-[1200px] mx-auto px-6 flex items-center justify-between h-16 md:h-[72px]">
          ...
          <a href="#" onClick={(e) => handleNavClick(e, '#')} className="flex items-center gap-2.5 shrink-0 group">
            ...
            <span className="text-[15px] text-white/90 tracking-[-0.01em]">
              Диалог<span className="text-white/40 ml-0.5">Эксперт</span>
            </span>
          </a>
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item, i) => (
              <motion.a
                ...
                className="relative px-3.5 py-2 text-[13px] text-white/50 hover:text-white/90 transition-colors duration-200 tracking-[-0.01em] group"
              >
                {item.label}
          ...
          <div className="flex items-center gap-3">
            <motion.a
              ...
              whileTap={{ scale: 0.97 }}
              className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#00A84F] hover:bg-[#00C85A] text-[13px] text-white tracking-[-0.01em] transition-all duration-200 shadow-[0_0_20px_rgba(0,168,79,0.3)] hover:shadow-[0_0_30px_rgba(0,168,79,0.5)]"
            >
              Получить расчёт
              <svg className="w-3.5 h-3.5 opacity-70" fill="none" viewBox="0 0 12 12">
            ...
            <nav className="max-w-[1200px] mx-auto px-6 py-4 flex flex-col gap-1">
              {navItems.map((item, i) => (
                <motion.a
                  ...
                  className="px-3 py-2.5 text-sm text-white/60 hover:text-white/90 transition-colors rounded-lg hover:bg-white/[0.04]"
                >
                  {item.label}
                </motion.a>
              ))}
              <motion.a
                ...
                className="mt-2 flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-[#00A84F] text-sm text-white"
              >
                Получить расчёт
              </motion.a>
            </nav>
